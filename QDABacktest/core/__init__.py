...
from QDABacktest.utils.array_utils import *
import sys
import time
...
def make_periodic_chunks(self, rebalance_period, timestamp_lst):
    idx = pd.Series(qbt.utils.array_utils.to_datetime(timestamp_lst))
    idx = pd.Series(pd.DataFrame({"a" : np.arange(len(idx))}, index=idx)["a"].resample(rebalance_period).max().index)
    idx = pd.Series.apply(lambda x: x.timestamp).values

    idx = [np.where(timestamp_lst == x)[0].item() for x in idx]

    return [timestamp_lst[idx[i]-1 if idx[i] > 0 else 0:idx[i+1]] for i in range(len(idx)-1)]



class DistributedRebalancingBacktestEngine(BacktestEngine):
    def __init__(self, rebalance_period):
        self.engines = [BacktestEngineWorker.remote(use_tqdm=False)]
